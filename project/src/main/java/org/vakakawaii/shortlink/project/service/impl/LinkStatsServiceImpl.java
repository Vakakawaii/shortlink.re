...
public class LinkStatsServiceImpl implements LinkStatsService {
    ...
    public LinkStatsRespDTO oneLinkStats(LinkStatsReqDTO linkStatsReqDTO) {
        ...
        // 地区访问详情（仅国内）
        List<LinkStatsLocateCNRespDTO> locateCnStats = new ArrayList<>();
        ...
        listedLocaleByShortLink.forEach(each -> {
            ...
            LinkStatsLocateCNRespDTO localeCNRespDTO = LinkStatsLocateCNRespDTO.builder()
                    .cnt(each.getCnt())
    ...
    @Override
    public LinkStatsRespDTO groupLinkStats(LinkGroupStatsReqDTO linkGroupStatsReqDTO) {
        List<LinkAccessStatsDO> listStatsByGroup = linkAccessStatsMapper.listStatsByGroup(linkGroupStatsReqDTO);
        if (CollUtil.isEmpty(listStatsByGroup)) {
            return null;
        }
        // 基础访问数据
        LinkAccessStatsDO pvUvUidStatsByGroup = linkAccessLogsMapper.findPvUvUidStatsByGroup(linkGroupStatsReqDTO);
        // 基础访问详情
        List<LinkStatsAccessDailyRespDTO> daily = new ArrayList<>();
        List<String> rangeDates = DateUtil.rangeToList(DateUtil.parse(linkGroupStatsReqDTO.getStartDate()), DateUtil.parse(linkGroupStatsReqDTO.getEndDate()), DateField.DAY_OF_MONTH).stream()
                .map(DateUtil::formatDate)
                .toList();
        ...
        // 地区访问详情（仅国内）
        List<LinkStatsLocateCNRespDTO> locateCnStats = new ArrayList<>();
        List<LinkLocateStatsDO> listedLocaleByGroup = linkLocateStatsMapper.listLocateByGroup(linkGroupStatsReqDTO);
        ...
        listedLocaleByGroup.forEach(each -> {
            double ratio = (double) each.getCnt() / localeCnSum;
            double actualRatio = Math.round(ratio * 100.0) / 100.0;
            LinkStatsLocateCNRespDTO locateCNRespDTO = LinkStatsLocateCNRespDTO.builder()
                    .cnt(each.getCnt())
                    .locate(each.getProvince())
                    .ratio(actualRatio)
                    .build();
        ...
        // 小时访问详情
        List<Integer> hourStats = new ArrayList<>();
        List<LinkAccessStatsDO> listHourStatsByGroup = linkAccessStatsMapper.listHourStatsByGroup(linkGroupStatsReqDTO);
        for (int i = 0; i < 24; i++) {
            AtomicInteger hour = new AtomicInteger(i);
        ...
        // 高频访问IP详情
        List<LinkStatsTopIpRespDTO> topIpStats = new ArrayList<>();
        List<HashMap<String, Object>> listTopIpByGroup = linkAccessLogsMapper.listTopIpByGroup(linkGroupStatsReqDTO);
        ...
        // 一周访问详情
        List<Integer> weekdayStats = new ArrayList<>();
        List<LinkAccessStatsDO> listWeekdayStatsByGroup = linkAccessStatsMapper.listWeekdayStatsByGroup(linkGroupStatsReqDTO);
        for (int i = 1; i < 8; i++) {
            AtomicInteger weekday = new AtomicInteger(i);
        ...
        // 浏览器访问详情
        List<LinkStatsBrowserRespDTO> browserStats = new ArrayList<>();
        List<HashMap<String, Object>> listBrowserStatsByGroup = linkBrowserStatsMapper.listBrowserStatsByGroup(linkGroupStatsReqDTO);
        ...
        // 操作系统访问详情
        List<LinkStatsOsRespDTO> osStats = new ArrayList<>();
        List<HashMap<String, Object>> listOsStatsByGroup = linkOsStatsMapper.listOsStatsByGroup(linkGroupStatsReqDTO);
        ...
        // 访问设备类型详情
        List<LinkStatsDeviceRespDTO> deviceStats = new ArrayList<>();
        List<LinkDeviceStatsDO> listDeviceStatsByGroup = linkDeviceStatsMapper.listDeviceStatsByGroup(linkGroupStatsReqDTO);
        ...
        // 访问网络类型详情
        List<LinkStatsNetworkRespDTO> networkStats = new ArrayList<>();
        List<LinkNetworkStatsDO> listNetworkStatsByGroup = linkNetworkStatsMapper.listNetworkStatsByGroup(linkGroupStatsReqDTO);
    ...
    @Override
    public IPage<LinkStatsAccessRecordRespDTO> groupLogLinkAccess(LinkGroupStatsAccessRecordReqDTO linkGroupStatsAccessRecordReqDTO) {
        IPage<LinkAccessLogsDO> linkAccessLogsDOIPage = linkAccessLogsMapper.selectGroupPage(linkGroupStatsAccessRecordReqDTO);
        ...
        List<Map<String, Object>> uvTypeList = linkAccessLogsMapper.selectGroupUvTypeByUsers(
                linkGroupStatsAccessRecordReqDTO.getGid(),
                linkGroupStatsAccessRecordReqDTO.getStartDate(),
                linkGroupStatsAccessRecordReqDTO.getEndDate(),
                userAccessLogsList
        );
